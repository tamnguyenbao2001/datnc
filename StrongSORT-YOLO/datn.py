    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_txt:
                            ...
                            if i == 0:
                                with open('E:\da2baotam\da2\StrongSORT-YOLO\dataframe.txt', 'a') as f:
                                    f.write(('%g ' * 12 + '\n') % (frame_idx + 1, cls, id, bbox_left,  # MOT format, stt_zone: So thu tu zone
                                                                bbox_top, bbox_w, bbox_h,a_stt_zone, -1, -1, -1, -1))
                            if i == 1:
                                with open('E:\da2baotam\da2\StrongSORT-YOLO\dataframecam2.txt', 'a') as f:
                                    f.write(('%g ' * 12 + '\n') % (frame_idx + 1, cls, id, bbox_left,  # MOT format, stt_zone: So thu tu zone
                                                                bbox_top, bbox_w, bbox_h,a_stt_zone, -1, -1, -1, -1))
            ...
            if count:
                ...
                try:
                    if i == 0:
                        df = pd.read_csv('E:\da2baotam\da2\StrongSORT-YOLO\dataframe.txt' , header=None, delim_whitespace=True)
                    if i == 1:
                        df = pd.read_csv('E:\da2baotam\da2\StrongSORT-YOLO\dataframecam2.txt' , header=None, delim_whitespace=True)
                ...
                with open('E:\da2baotam\da2\StrongSORT-YOLO\dataframe.txt','w') as file:    ##### Xoa file txt de khong luu lai gia tri count, neu la video thi lui` lai 1 tab`
                    pass
                with open('E:\da2baotam\da2\StrongSORT-YOLO\dataframecam2.txt','w') as file:    ##### Xoa file txt de khong luu lai gia tri count, neu la video thi lui` lai 1 tab`
                    pass
